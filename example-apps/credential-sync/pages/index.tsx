...
export default function Index() {
  ...
  return (
    <div>
      ...
      <button onClick={() => updateToken({ invalid: true })}>Give an invalid token to Null Chapter VITAP</button>
      <button onClick={() => updateToken()}>Give a valid token to Null Chapter VITAP</button>
      <div>{data}</div>
    </div>
  );
}
